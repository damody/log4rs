...
impl Append for MqttAppender {
    fn append(&self, record: &Record) -> anyhow::Result<()> {
        ...
        // Debug: Print MQTT publish attempt
        eprintln!("[MQTT Debug] Attempting to publish to topic '{}' with QoS {:?}", topic, self.qos);
        eprintln!("[MQTT Debug] Message payload: {}", String::from_utf8_lossy(&buffer.0));
        
        // Send the message
        let client = self.client.lock();
        match client.publish(&topic, self.qos, false, buffer.0) {
            Ok(_) => {
                eprintln!("[MQTT Debug] Message published successfully");
                Ok(())
            }
            Err(e) => {
                eprintln!("[MQTT Debug] Failed to publish message: {:?}", e);
                Err(e.into())
            }
        }
    }
...
impl MqttAppenderBuilder {
    ...
    pub fn build(self) -> io::Result<MqttAppender> {
        ...
        eprintln!("[MQTT Debug] Connecting to broker: {}:{}", host, port);
        eprintln!("[MQTT Debug] Client ID: {}", self.client_id);
        eprintln!("[MQTT Debug] Topic template: {}", self.topic);
        ...
        if let (Some(ref username), Some(ref password)) = (self.username, self.password) {
            eprintln!("[MQTT Debug] Using authentication with username: {}", username);
        ...
        eprintln!("[MQTT Debug] MQTT client created, starting connection handler");
        
        // Start connection handler in background thread
        thread::spawn(move || {
            eprintln!("[MQTT Debug] Connection handler thread started");
            // Handle connection events
            for notification in connection.iter() {
                match notification {
                    Ok(Event::Incoming(Packet::ConnAck(connack))) => {
                        eprintln!("[MQTT Debug] Connected successfully! ConnAck: {:?}", connack);
                    }
                    Ok(Event::Incoming(packet)) => {
                        eprintln!("[MQTT Debug] Received packet: {:?}", packet);
                    }
                    Ok(Event::Outgoing(packet)) => {
                        eprintln!("[MQTT Debug] Sent packet: {:?}", packet);
                    }
                    Err(e) => {
                        eprintln!("[MQTT Debug] Connection error: {:?}", e);
                        // Connection will automatically retry
                    }
                }
            }
            eprintln!("[MQTT Debug] Connection handler thread ended");
        });
